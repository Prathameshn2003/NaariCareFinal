...
const dashboardSections = [
  ...
  },
  {
    icon: Building2,
    title: "NGOs & Support",
    description: "Find support organizations",
    path: "/ngos",
    color: "text-primary",
  ...
  },
  {
    icon: FileText,
    title: "Govt. Schemes",
    description: "Explore health benefits",
    path: "/schemes",
    color: "text-accent",
    ...
    <DashboardLayout>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
        {quickActions.map((action) => (
          <Link
            key={action.label}
            to={action.path}
            className="glass-card rounded-xl p-4 flex flex-col items-center gap-3 hover:shadow-glow transition-all hover:-translate-y-1 group"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        {healthCards.map((card) => (
          <Link
            key={card.title}
            to={card.path}
            className="glass-card rounded-2xl p-6 hover:shadow-glow transition-all group"
          >
            ...
            <p className={`text-sm font-medium ${card.statusColor} mb-4`}>
              {card.status}
            </p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {dashboardSections.map((section) => (
            <Link
              key={section.title}
              to={section.path}
              className="glass-card rounded-xl p-4 hover:shadow-glow transition-all text-center"
            >
              <div
                className={`w-10 h-10 rounded-lg ${section.bgColor} flex items-center justify-center mx-auto mb-3`}
              >
      ...
      <div className="glass-card rounded-2xl p-6 md:p-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Track your menstrual health patterns
            </p>
          </div>
          <Link to="/modules/menstrual">
        ...
        <div className="flex flex-wrap gap-4 mt-6 pt-4 border-t border-border">
          <Legend color="bg-primary/40" label="Period" />
          <Legend color="bg-teal/30" label="Ovulation Window" />
